...
&& curl -s "https://get.sdkman.io" | bash && chmod +x "/root/.sdkman/bin/sdkman-init.sh" && source "$HOME/.sdkman/bin/sdkman-init.sh" && sdk install java 25-tem \ && jlink --compress=zip-9 --no-header-files --strip-java-debug-attributes --no-man-pages --vm=server --module-path "$HOME/.sdkman/candidates/java/current/jmods" --add-modules ALL-MODULE-PATH --output=./jre \
&& sdk uninstall --force java 25-tem && rm -rf ~/.sdkman  \
