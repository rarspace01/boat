...
HEALTHCHECK CMD curl -f http://localhost:8080/ || bash -c 'kill -s 15 -1 && (sleep 10; kill -s 9 -1)'
VOLUME /tmp/boat
ENV JAVA_HOME="./jdk"
RUN apt-get update && apt-get install -y curl wget unzip dnsutils \
&& curl https://raw.githubusercontent.com/sormuras/bach/master/install-jdk.sh -o install-jdk.sh && chmod +x install-jdk.sh;./install-jdk.sh -v -o linux-`uname -m` -f 17 --target ./jdk \
